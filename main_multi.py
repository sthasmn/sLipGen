#this is test version. Might not work. Confirming it.
"""
A module implementing a multi-process pipeline for video scene detection,
face detection, tracking, transcription, and clip generation.

This version uses a multiprocessing pool to parallelize the CPU-bound
pre-processing (FFmpeg frame and audio extraction) to keep the GPU
continuously fed with data, maximizing throughput for batch processing.
"""
...
class Config:
    ...
    # --- Process Management ---
    # Number of CPU workers for pre-processing.
    # A value of os.cpu_count() - 2 is a safe starting point.
    NUM_CPU_WORKERS: int = max(1, os.cpu_count() - 2)
...
# =====================================================================================
#  CPU Worker Function (for Multiprocessing Pool)
# =====================================================================================

def preprocess_video_worker(video_path: Path):
    """
    Worker function to be executed in a separate process.
    Handles the CPU-heavy FFmpeg pre-processing tasks.
    """
    try:
        ...
        os.makedirs(pyavi_path, exist_ok=True)
        os.makedirs(pyframes_path, exist_ok=True)

        # 1. Extract Audio
        audio_path = pyavi_path / 'audio.wav'
        subprocess.run(
            ['ffmpeg', '-y', '-i', str(video_path), '-qscale:a', '0', '-ac', '1', '-vn', '-ar', '16000',
             str(audio_path), '-loglevel', 'panic'],
            check=True
        )
        ...
        if not audio_path.exists() or audio_path.stat().st_size == 0:
            return False, (video_path, "Failed to create a valid audio file.")

        if not any(pyframes_path.iterdir()):
            return False, (video_path, "Failed to extract any frames.")
...
# =====================================================================================
#  Core Processing Functions (mostly run in main process)
# =====================================================================================
...
# crop_video, evaluate_network, create_final_clip
...
def inference_video(frames_dir: Path, work_dir: Path, config: Config):
    ...
    dets = []
    # Using tqdm for a cleaner progress bar
    for fidx, fname in enumerate(tqdm.tqdm(flist, desc=f"[{frames_dir.parent.name}] Face Det")):
...
def track_shot(scene_faces, config: Config):
    ...
    while True:
        ...
        if len(track) > config.MIN_TRACK:
            ...
            bboxes_i = []
            for ij in range(4):
                interpfn = interp1d(frame_num, bboxes[:, ij], kind='linear', bounds_error=False, fill_value="extrapolate")
                bboxes_i.append(interpfn(frame_i))
            bboxes_i = np.stack(bboxes_i, axis=1)
            if max(np.mean(bboxes_i[:, 2] - bboxes_i[:, 0]), np.mean(bboxes_i[:, 3] - bboxes_i[:, 1])) > config.MIN_FACE_SIZE:
...
def crop_video(track, frames_dir: Path, audio_path: Path, crop_file_path: Path, config: Config, fps: float):
    ...
    for fidx, frame_num in enumerate(track['frame']):
        ...
        if image is None: continue
        cs, bs = config.CROP_SCALE, dets['s'][fidx]
        bsi = int(bs * (1 + 2 * cs))
        ...
        vOut.write(cv2.resize(face, (224, 244)))
    vOut.release()
...
def evaluate_network(pycrop_path: Path, asd_model: ASD, config: Config):
    ...
    all_scores = []
    for file in tqdm.tqdm(files, desc=f"[{pycrop_path.parent.name}] ASD Scoring"):
        try:
...
def create_final_clip(track_data, start_time, end_time, frames_dir, audio_path, output_path, config, fps):
    ...
    try:
        ...
        if not all_frame_files: raise FileNotFoundError("No frames found.")
        ...
        for frame_num in range(start_frame, end_frame + 1):
            if frame_num >= len(all_frame_files): continue
            if frame_num in track_frames_map:
                ...
                bsi = int(bs * (1 + 2 * cs))
                frame_padded = np.pad(image, ((bsi, bsi), (bsi, bsi), (0, 0)), 'constant', constant_values=(110, 110))
                my, mx = proc_track['y'][track_idx] + bsi, proc_track['x'][track_idx] + bsi
                y1, y2 = int(my - bs), int(my + bs * (1 + 2 * cs))
                x1, x2 = int(mx - bs * (1 + cs)), int(mx + bs * (1 + cs))
                face = frame_padded[y1:y2, x1:x2]
        ...
        cmd_mux_list = ['ffmpeg', '-y', '-i', str(tmp_video_path), '-i', str(tmp_audio_path), '-c:v', 'libx264', '-c:a', 'copy', str(output_path), '-loglevel', 'panic']
        subprocess.run(cmd_mux_list, check=True)
    except Exception as e:
        print(f"  - FAILED to create clip for {output_path.name}. Reason: {e}")
    finally:
...
# NOTE: To save space, I'll assume all functions from your original script are copied here.
# I'll paste them at the end of this response block for completeness.

def process_video_main(video_path: Path, config: Config, whisper_model: whisper.Whisper, asd_model: ASD):
    """
    This function runs the main, sequential part of the pipeline for a single video
    AFTER it has been successfully pre-processed.
    """
    base_output_dir = video_path.parent / video_path.stem
    pyframes_path = base_output_dir / 'pyframes'
    pywork_path = base_output_dir / 'pywork'
    pycrop_path = base_output_dir / 'pycrop'
    audio_path = base_output_dir / "pyavi" / "audio.wav"

    os.makedirs(pywork_path, exist_ok=True)
    os.makedirs(pycrop_path, exist_ok=True)
    ...
    # --- ASD Pipeline ---
    print(f"\n[{video_path.name}] Running face detection...")
    ...
    print(f"[{video_path.name}] Tracker found {len(all_tracks)} tracks.")

    vid_tracks = []
    for i, track in enumerate(tqdm.tqdm(all_tracks, desc=f"[{video_path.name}] Cropping")):
    ...
    # --- Whisper Transcription and Final Muxing ---
    print(f"[{video_path.name}] Transcribing audio...")
    ...
    for seg_idx, segment in enumerate(transcription["segments"]):
        ...
        for track_idx, track_scores in enumerate(scores):
            if not track_scores: continue
            track_start_frame = vid_tracks[track_idx]['track']['frame'][0]
            track_end_frame = vid_tracks[track_idx]['track']['frame'][-1]
        ...
        if best_track_idx != -1 and best_avg_score > config.MIN_SPEAKER_SCORE_THRESHOLD:
            ...
            create_final_clip(
                track_data=vid_tracks[best_track_idx],
                start_time=start_time,
                end_time=end_time,
                frames_dir=pyframes_path,
                audio_path=audio_path,
                output_path=clip_path,
                config=config,
                fps=fps
            )
            if clip_path.exists():  # Ensure clip was created before writing text and manifest
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python main_multi.py /path/to/videos/ [skip-asd]")
        sys.exit(1)
    ...
    print(f"[ASD] Loading model from '{config.PRETRAIN_MODEL_PATH}' on {config.DEVICE}...")
    asd_model = ASD().to(config.DEVICE)
    asd_model.loadParameters(config.PRETRAIN_MODEL_PATH)
    asd_model.eval()
    print("[ASD] Model loaded.")
    ...
    print(f"Found {len(video_files)} video(s) to process using {config.NUM_CPU_WORKERS} CPU workers.")

    # --- Filter out already processed videos ---
    if flag == "skip-asd":
        print("[Filter] Removing videos with existing manifests...")
        filtered_videos = []
        for video_path in video_files:
            manifest_path = video_path.parent / video_path.stem / "output" / "manifest.json"
            if manifest_path.is_file():
                print(f"  - Skipping {video_path.name}, manifest found.")
            else:
                filtered_videos.append(video_path)
        video_files = filtered_videos
        print(f"[Filter] {len(video_files)} videos remaining to be processed.")
    ...
    with multiprocessing.Pool(processes=config.NUM_CPU_WORKERS) as pool:
        # Create a progress bar for the pre-processing stage
        pbar = tqdm.tqdm(total=len(video_files), desc="[Pre-processing]")

        # `imap_unordered` gets results as they are completed
        for success, result in pool.imap_unordered(preprocess_video_worker, video_files):
            pbar.update(1)
            if success:
                video_path = result
                try:
                    # Now run the main (mostly GPU) part of the pipeline
                    process_video_main(video_path, config, whisper_model, asd_model)
                except Exception as e:
                    print(f"!!! Error during main processing of {video_path.name}: {e}")
                    traceback.print_exc()
            else:
                video_path, error_msg = result
                print(f"!!! Error pre-processing {video_path.name}: {error_msg}")
        pbar.close()
